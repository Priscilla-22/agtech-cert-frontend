    ...
    useEffect(() => {
        const fetchInspection = async () => {
            try {
                ...
                // Fetch farm and farmer details using the farmId
                if (inspectionData.farmId) {
                    const farmResponse = await fetch(`/api/farms/${inspectionData.farmId}`)

                    if (farmResponse.ok) {
                        const farmData = await farmResponse.json()
                        setFarm(farmData)

                        // Now fetch farmer data using the farmerId from farm data
                        if (farmData.farmerId) {
                            const farmerResponse = await fetch(`/api/farmers/${farmData.farmerId}`)
                    ...
                    <main className="flex-1 p-3 sm:p-4 md:p-6">
                        <div className="max-w-7xl mx-auto space-y-4 md:space-y-6 pb-20 md:pb-96">
                            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                                <div>
                                    <h1 className="text-2xl sm:text-3xl font-space-grotesk font-bold text-foreground">Inspection
                                        Details</h1>
                                    <p className="text-sm sm:text-base text-muted-foreground">
                                        {farm?.farmName || 'Loading...'} - {farmer?.name || 'Loading...'}
                                    </p>
                                </div>
                            ...
                            <div className="grid gap-6 md:grid-cols-2">
                                <Card className="border-0 shadow-sm">
                                    ...
                                    <CardContent className="space-y-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="font-medium">{farm?.farmName || 'Loading...'}</p>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="font-medium">{farmer?.name || 'Loading...'}</p>
                                ...
                                <Card className="border-0 shadow-sm">
                                        ...
                                        {inspection.score ? (
                                            <>
                                                <div className="text-center">
                                                    <div className="text-4xl font-bold mb-2">{inspection.score}/100
                                                    </div>
                                                    <Badge variant={inspection.score >= 80 ? "default" : "destructive"}
                                                           className="text-sm">
                                  ...
                                  {inspection.checklist.filter((item: { id: string; answer: boolean }) =>
                                  ...
                                  {inspection.checklist.filter((item: { id: string; answer: boolean }) =>
                                      ['bufferZones', 'compostManagement'].includes(item.id) && item.answer === true
                                                            ...
                                                            <div className="flex justify-between text-sm">
                                                                ...
                                                                <span
                                                                    className="font-medium">{inspection.score}/100</span>
                                                            </div>
                                                        </>
                                                    ) : (
                                ...
                                <Card className="border-0 shadow-sm">
                                    ...
                                    <CardContent>
                                        <ul className="space-y-2">
                                            {inspection.violations.map((violation: string | number | bigint | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<AwaitedReactNode> | null | undefined, index: Key | null | undefined) => (
